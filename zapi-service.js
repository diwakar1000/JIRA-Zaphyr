...
const zqlSearch = function(query) {
...
const getExecutionStatuses = function() {
...
const getExecutionsForIssue = function(issueKey) {
    return zqlSearch("ISSUE = " + issueKey).then((result) => {
        return result;
    });
}

const deleteAllExecutionsForIssue = function(issueKey) {
...
const getIssueIdFromIssueKey = function(issueKey) {
    return zqlSearch("ISSUE = " + issueKey).then((result) => {
        return result.tests[0].issueId;
    });
}

const getCycleFromCycleName = function(jiraProjectId, jiraProjectVersion, cycleName) {
...
const createAdHocExecution = function(issueKey, projectId) {
    //find the issue key
    return getIssueIdFromIssueKey(issueKey).then((issueId) => {
        body = {
            "projectId": projectId,
...
const updateExecutionStatus = function(executionId, issueKey, projectId, executionStatus) {
    return getIssueIdFromIssueKey(issueKey).then((issueId) => {
        body = {
            "projectId": projectId,
            "issueId": issueId,
